...
class GUI:
    ...
    def click_event(self, row, column):
        ...
        if self.first_click is (row, column):
        ...
        if self.answer[row][column] == self.answer[x][y]:
            ...
            '''if self.second_click is (row, column):
                n, m = self.second_click
            else:
                print("nothing")

            if self.answer[row][column] == self.answer[n][m]:
                print("match!")
                self.buttons[n][m].config(state=DISABLED)
                self.answer[row][column] = ''
                self.answer[n][m] = ''
            else:
                 self.middle_frame.after(2000, self.reset_buttons, row, column, n, m)
                 print("No match! You suck!")
                 self.second_click = None'''
